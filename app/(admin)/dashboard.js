...
export default function HRMonolithicControlHub() {
  ...
  // 🧭 UI WORKSPACE VIEW STATE ('dbms' displays the grid table, 'tracking' displays the maps)
  ...
  return (
    <View style={styles.container}>
      {/* MONOLITHIC NAV DESK HEADER */}
      <View style={styles.header}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 16 }}>
          {currentTab === 'tracking' && (
            <TouchableOpacity style={styles.backBtn} onPress={() => setCurrentTab('dbms')}>
              <Text style={styles.textWhite}>📋 Back to DBMS Records</Text>
            </TouchableOpacity>
          )}
      ...
      <View style={styles.mainContent}>
        ...
        <View style={styles.sidebar}>
          ...
          <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false}>
            {filteredSidebarRoster.map((emp) => {
              ...
              const isSelected = selectedUser.id === emp.userId;
              return (
                <TouchableOpacity
                  key={emp.userId}
        ...
        {currentTab === 'dbms' ? (
          <View style={styles.workspace}>
            {/* VIEW A: MASTER CONTROL RELATIONAL DATA SHEET */}
            <View style={styles.controlBar}>
              ...
              <TextInput style={[styles.tableInputFilter, { width: 180, flex: 0 }]} placeholder="📍 Base Boundary..." placeholderTextColor="#94A3B8" value={areaFilter} onChangeText={setAreaFilter} />
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator style={styles.tableScroll}>
              <View style={{ padding: 12 }}>
                <div style={styles.tableHeaderRow}>
                  {['ID','First Name','Last Name','Base City','Current City','Freq Client 1','Freq Client 2','Freq Client 3','Email','Mobile','Month Days','Actions'].map((h,i)=><Text key={i} style={[styles.th, { width: i===8?180:i===11?240:110 }]}>{h}</Text>)}
                </div>
                  ...
                  {filteredGridRoster.map((emp, idx) => {
                    ...
                    return (
                      <View key={emp._id || idx} style={[styles.tableRow, idx % 2 === 1 && styles.rowAlternate]}>
                        <Text style={[styles.td, { width: 110, fontWeight: '700' }]}>{emp.empId}</Text>
                        <Text style={[styles.td, { width: 110 }]}>{emp.firstName || emp.name}</Text>
                        <Text style={[styles.td, { width: 110 }]}>{emp.lastName || '-'}</Text>
                        <Text style={[styles.td, { width: 110, color: '#475569' }]}>Gurugram</Text>
                        <View style={{ width: 110 }}><View style={[styles.badge, isTravelling ? styles.badgeTravel : styles.badgeHome]}><Text style={{fontSize:11, fontWeight:'700', color:isTravelling?'#0369A1':'#166534'}}>📍 {isTravelling?emp.area:'Gurugram'}</Text></View></View>
                        <Text style={[styles.td, { width: 110, color: '#0EA5E9', fontWeight: '700' }]}>{emp.freqClient1}</Text>
                        <Text style={[styles.td, { width: 110 }]}>{emp.freqClient2}</Text>
                        <Text style={[styles.td, { width: 110 }]}>{emp.freqClient3}</Text>
                        {isEditing ? <TextInput style={styles.inlineInput} value={editFields.email} onChangeText={t=>setEditFields({...editFields, email:t})} /> : <Text style={[styles.td, { width: 180, fontSize: 12 }]}>{emp.email}</Text>}
                        {isEditing ? <TextInput style={styles.inlineInput} value={editFields.mobile} onChangeText={t=>setEditFields({...editFields, mobile:t})} /> : <Text style={[styles.td, { width: 110 }]}>{emp.mobile}</Text>}
          ...
          <View style={styles.workspaceSplit}>
            {/* VIEW B: TRACKING TIMELINE WORKSPACE */}
              ...
              <ScrollView style={styles.listBlock}>
                {visitNotes.map((n,i) => (
                  <View key={i} style={styles.noteCard}>
                    <Text style={{fontWeight:'700', fontSize:13}}>🏢 Client: {n.clientName}</Text>
...
const styles = StyleSheet.create({
  ...
  searchBarContainer: { backgroundColor: '#F8FAFC', padding: 8, borderRadius: 6, borderWidth: 1, borderColor: '#E2E8F0', marginBottom: 12 },
  ...
  tableInputFilter: { flex: 1, backgroundColor: '#F8FAFC', padding: 8, borderRadius: 6, borderWidth: 1, borderColor: '#E2E8F0' },
  ...
  inlineInput: { width: 110, borderWidth: 1, borderColor: '#3B82F6', borderRadius: 4, padding: 4, backgroundColor: '#FFF' },
  ...
  viewBtn: { backgroundColor: '#EFF6FF', padding: 6, borderRadius: 4 },
  editBtn: { backgroundColor: '#F1F5F9', padding: 6, borderRadius: 4 },
  saveBtn: { backgroundColor: '#22C55E', padding: 6, borderRadius: 4 },
  cancelBtn: { backgroundColor: '#E2E8F0', padding: 6, borderRadius: 4 },
  historyPanel: { width: 320, borderRightWidth: 1, borderColor: '#E2E8F0', backgroundColor: '#F8FAFC', padding: 14 },
  ...
  listBlock: { maxHeight: 140, minHeight: 80, backgroundColor: '#FFFFFF', borderRadius: 8, borderWidth: 1, borderColor: '#E2E8F0', padding: 6 },
  ...
  dropdownSelector: { padding: '4px 8px', borderRadius: '6px', backgroundColor: '#F8FAFC', border: '1px solid #E2E8F0', fontWeight: '700' },
